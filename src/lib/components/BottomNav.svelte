<script lang="ts">
	...
	// `unread` puts a small badge on the Assistant item (unread report chats).
...
>
	<div class="mx-auto grid h-16 max-w-md grid-cols-3 items-center px-6">
		...
		<a
			...
			style="color: {active.startsWith('/trends')
				? 'var(--color-accent-to, #fb923c)'
		...
		<a
			href="/reports"
			class="flex flex-col items-center gap-1 transition active:scale-95"
			style="color: {active.startsWith('/reports')
				? 'var(--color-accent-to, #fb923c)'
				: 'var(--color-text-subtle)'};"
		...
		>
			<span class="relative">
				...
				{#if unread > 0}
					<span
						class="accent-gradient absolute -top-1.5 -right-2.5 flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[9px] leading-none font-bold text-white"
					>
						{unread > 9 ? '9+' : unread}
					</span>
				{/if}
