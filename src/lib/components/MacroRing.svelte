<script lang="ts">
	...
	let { value, target, size = 170, showRemaining = true, ontoggle }: Props = $props();
	...
	// With a target set we can show either what's left or what's been consumed;
	// the "left/over" framing only applies to the remaining view.
	let remaining = $derived(target - value);
	let over = $derived(hasTarget && remaining < 0);
	let showLeft = $derived(hasTarget && showRemaining);
	let bigNumber = $derived(
		(showLeft ? Math.round(Math.abs(remaining)) : Math.round(value)).toLocaleString()
	);
	let subLabel = $derived(showLeft ? `kcal ${over ? 'over' : 'left'}` : 'kcal');
</script>
...
{#if ontoggle}
	<button
		...
		aria-label={`Calories ${showLeft ? 'left' : 'consumed'} — tap to toggle`}
	>
		{@render ring()}
	</button>
{:else}
