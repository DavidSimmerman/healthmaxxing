...
| `MCP_SERVICE_TOKEN`       | for reports      | Strong static **read/write** bearer that lets the trusted sidecar call `/mcp` without the interactive OAuth flow. Must equal `MCP_TOKEN`. Unset = only OAuth tokens are accepted.                                                         |
| `MCP_TOKEN`               | for reports      | Same value as `MCP_SERVICE_TOKEN` (the sidecar reads this name; used by `/report`, which writes via `save_report`).                                                                                                                       |
| `MCP_SERVICE_TOKEN_RO`    | for chat         | **Read-only** variant: `/mcp` refuses the mutating tools (`log_food`, `save_report`, …) on this bearer. Must equal `MCP_TOKEN_RO`. Defense in depth for chat — a prompt-injected label can't write even if the sidecar allowlist slipped. |
| `MCP_TOKEN_RO`            | for chat         | Same value as `MCP_SERVICE_TOKEN_RO` (the sidecar reads this name; used by `/chat`). Unset = the sidecar falls back to `MCP_TOKEN` for chat (works, but loses the read-only scoping).                                                     |
| `APP_MCP_URL`             | for reports/chat | `http://127.0.0.1:3000/mcp` — the app's own MCP over loopback.                                                                                                                                                                            |
| `BODY_SIZE_LIMIT`         | for photos       | adapter-node's max request body (default **512KB**). Photos arrive as base64 JSON (~4/3 the raw bytes) and the sidecar accepts 20MB bodies (`MAX_BODY`), so set `20M` to match. Without it, image uploads fail.                           |
