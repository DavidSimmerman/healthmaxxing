...
try {
	...
	await p
		.getByRole('button', { name: /what about oatmeal/ })
		.first()
	...
	await p
		...
		.click();
	await p
		.getByText('Hey! Oatmeal is about 150 calories.', { exact: false })
		.waitFor({ timeout: 8000 });
	await p.getByText('Logged', { exact: false }).waitFor({ timeout: 8000 });
	...
	log('FAIL:', e.message);
	await p.screenshot({ path: '/home/claude/hm-verify/fail.png' }).catch(() => {});
	process.exitCode = 1;
} finally {
	await b.close();
}
